...
namespace CrimeManagementSystem.Services
{
    public class OfficerService : IOfficerService
    {
        ...
        // ADD OFFICER (Station Head) (CUSTOM EXCEPTION => DuplicateResourceException)
        ...
        public async Task<OfficerResponseDTO> AddOfficerAsync(AddOfficerDTO dto)
        {
            try
            {
                ...
                if (existing != null)
                {
                    ...
                    throw new Exceptions.DuplicateResourceException("Officer with this email already exists!");
                }
                ...
                if (existingBadge != null)
                {
                    ...
                    throw new Exception("Badge number already exists!");
                }
                ...
                _logger.LogInformation("New officer added: {OfficerId} - {Badge}", officer.OfficerId, officer.BadgeNumber);
        ...
        public async Task<OfficerResponseDTO> GetOfficerByIdAsync(int officerId)
        {
            ...
            catch (Exception ex) when (ex.Message != "Officer not found!")
            {
        ...
        public async Task<bool> RemoveOfficerAsync(int officerId)
        {
            try
            {
                ...
                if (activeIncidents > 0)
                {
                    ...
                    throw new Exception($"Cannot remove officer with {activeIncidents} active incidents!");
                }
            ...
            catch (Exception ex) when (!ex.Message.StartsWith("Officer not found") && !ex.Message.StartsWith("Cannot remove officer"))
            {
