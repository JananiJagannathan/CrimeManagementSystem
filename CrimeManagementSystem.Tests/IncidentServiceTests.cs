...
namespace CrimeManagementSystem.Tests
{
    ...
    public class IncidentServiceTests
    {
        ...
        public void Setup()
        {
            ...
            _incidentService = new IncidentService(_context, _mapper, _loggerMock.Object);
        }
        ...
        [Test]
        public async Task AssignOfficerAsync_InvalidIncidentId_ThrowsException()
        {
            // Arrange
            var officerId = await CreateTestOfficerAsync();
            var assignDto = new AssignOfficerDTO
            {
                IncidentId = 9999, // doesn't exist
                OfficerId = officerId
            };
            ...
            Assert.That(ex!.Message, Is.EqualTo("Incident not found!"));
        }
        ...
        public async Task VerifyIncidentAsync_NotClosedYet_ThrowsException()
        {
            ...
            // Act & Assert (incident is still "Initiated", not "Closed")
            var ex = Assert.ThrowsAsync<Exception>(async () =>
                await _incidentService.VerifyIncidentAsync(incident.IncidentId));

            Assert.That(ex!.Message, Is.EqualTo("Incident must be closed before verifying!"));
        }
        ...
        public async Task CloseIncidentAsync_NotAssignedToOfficer_ThrowsException()
        {
            ...
            // Act & Assert (officer tries to close an incident never assigned to them)
            var ex = Assert.ThrowsAsync<Exception>(async () =>
                await _incidentService.CloseIncidentAsync(incident.IncidentId, officerId));

            Assert.That(ex!.Message, Is.EqualTo("Incident not found or not assigned to you!"));
        }
