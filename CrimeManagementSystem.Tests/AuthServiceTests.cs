...
namespace CrimeManagementSystem.Tests
{
    ...
    public class AuthServiceTests
    {
        ...
        public void Setup()
        {
            ...
            // In-memory database (fresh DB for each test)
            var options = new DbContextOptionsBuilder<AppDbContext>()
                .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString())
                .Options;
            _context = new AppDbContext(options);

            // Fake configuration values (JWT settings needed by service)
            ...
            _authService = new AuthService(_context, _config, _loggerMock.Object, _emailServiceMock.Object);
        }
        ...
        public async Task RegisterAsync_DuplicateEmail_ThrowsException()
        {
            ...
            await _authService.RegisterAsync(dto); // first registration succeeds

            // Act & Assert
            var ex = Assert.ThrowsAsync<Exception>(async () =>
                await _authService.RegisterAsync(dto)); // second attempt, same email
        ...
        // TEST 9: Change Password with Correct Current Password
        [Test]
        ...
        // ── TEST 10: Change Password with Wrong Current Password Fails
        [Test]
